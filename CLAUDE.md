...
`docs/architecture.md` explains the three decisions everything follows from (virtual time, the job table, the event log). `docs/paystack.md` is the coverage contract. `CONTRIBUTING.md` lists the non-negotiables. `docs/README.md` indexes the user-facing documentation (concepts, providers, control API, configuration, test instruments, lifecycle, time, testing, dashboard, troubleshooting); keep those pages in step when behaviour they describe changes.
...
- **Flutterwave v4 webhooks follow the resource, not the formatter.** Everything the v4 API creates is marked `paybox_api_version: 'v4'` in metadata (`providers/flutterwave/src/v4/version.ts`); the single Flutterwave formatter reads the marker, builds v4's `{webhook_id, timestamp, type, data}` envelope (`v4/webhook.ts`, transcribed from Flutterwave's reference schemas) and returns `variant: 'v4'`, which the dispatcher hands back to `sign()` so those deliveries carry `flutterwave-signature`. `FormattedWebhook.variant` is the general seam for any provider with two wire formats. `publicMeta` strips the marker and the `v4_*` scenario keys from every `meta` the v4 API echoes.
